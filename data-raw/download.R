...
# article_metadata2 <- rcrossref::cr_works(dois_list[[1]], .progress = 'text')$data %>%
  # select(container.title, doi, issn, subject, title, publisher)
# readr::write_csv(article_metadata2, 'article_metadata2.csv')
library(dplyr)
num_left <- length(dois_list) - 1
for (chunk in dois_list) {
  Sys.sleep(3)
  message(num_left, ' chunks to go')
...
article_metadata <- article_metadata %>%
  ...
  left_join(group_by(., journal_id) %>%
            ...
            by = 'journal_id')
usethis::use_data(article_metadata, overwrite = TRUE)
